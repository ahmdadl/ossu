...
def getWordScore(word, n):
    ...
    # first try used all letters bonus
    if len(word) == n:
        score += 50
...
def updateHand(hand, word):
    ...
    for i in handChars:
        if w.count(i) > 0:
            ...
            handed[i] = handed.get(i) + 1 if isinstance(handed.get(i), int) else 1
...
def calculateHandlen(hand):
    ...
    returns: integer
    """
    return sum(hand.get(l, 0) for l in hand)
    ...
    while len(hand) != 0:    
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                print(f'"{userInput}" earned {totalScore} points. Total: {totalScore} points', end='\n')
...
def playGame(wordList):
    ...
    while True:
        ...
        if userInput == 'e':
            ...
            playHand(getFrequencyDict(newWord), wordList, HAND_SIZE)
        elif userInput == 'r':
            if len(handsPlayed) == 0:
                print('You have not played a hand yet. Please play a new hand first!')
            else:
                playHand(getFrequencyDict(handsPlayed[-1]), wordList, HAND_SIZE)
        else:
